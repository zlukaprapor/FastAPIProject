-- PostgreSQL migration for Travel Planner API (idempotent)
...
CREATE INDEX IF NOT EXISTS idx_travel_plans_public
    ON travel_plans(is_public, updated_at DESC)
    WHERE is_public = TRUE;

CREATE INDEX IF NOT EXISTS idx_travel_plans_updated
    ON travel_plans(updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_locations_plan_order
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_trigger
        WHERE tgname = 'update_travel_plans_updated_at'
    ) THEN
